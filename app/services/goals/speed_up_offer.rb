module Goals
  ...
  SpeedUpOffer = Data.define(:surplus_cents, :source_bank_account_id, :destination_bank_account_id) do
    def self.for(goal, month: Date.current.in_time_zone(TZ).to_date.beginning_of_month)
      ...
      surplus = MonthSummary.new(goal.account, month).remaining_cents
      return nil unless surplus.positive? && surplus * 5 >= goal.monthly_target_cents.to_i
      new(surplus_cents: surplus, source_bank_account_id: commitment.bank_account_id,
