...
class IncomesControllerTest < ActionDispatch::IntegrationTest
  setup do
    ...
    sign_in_as(@user)
    @account = @user.account.bank_accounts.create!(institution: Institution.find_by(code: "260"))
  end
  ...
  test "a forged bank_account_id of another user is rejected, no leak" do
    ...
    theirs = other.account.bank_accounts.create!(institution: Institution.find_by(code: "260"))
  ...
  test "cannot edit or update another user's income" do
    ...
    Accounts::Bootstrap.call(other)
    theirs = other.account.bank_accounts.create!(institution: Institution.find_by(code: "260"))
    inc = other.account.incomes.create!(bank_account: theirs, name: "alheia", amount_cents: 100,
                                schedule_kind: "fixed_day", schedule_day: 5)
    get edit_income_url(inc)
  ...
  test "receive posts a linked deposit for the month, once" do
    ...
    assert_no_difference -> { @user.account.transactions.posted.count } do
      patch receive_income_url(inc, month: month.strftime("%Y-%m")), as: :turbo_stream
    end
  end
  ...
  test "cannot receive another user's income" do
    other  = User.create!(email_address: "y@example.com", password: "password123")
    Accounts::Bootstrap.call(other)
    theirs = other.account.incomes.create!(bank_account: other.account.bank_accounts.create!(institution: Institution.find_by(code: "260")),
