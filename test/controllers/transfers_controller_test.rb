...
class TransfersControllerTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @checking = @user.bank_accounts.create!(institution: @inst, nickname: "Itaú", balance_cents: 100_000)
    @savings  = @user.bank_accounts.create!(institution: @inst, nickname: "Caixinha", kind: "savings", balance_cents: 0)
  end
  ...
  test "creates exactly one posted transfer row" do
    assert_difference -> { @user.transactions.where(direction: "transfer").count }, 1 do
  ...
  test "a transfer to savings bumps Guardado; a same-month resgate does NOT decrement it (gross)" do
    ...
    assert_equal 20_000, MonthSummary.new(@user, Date.current.beginning_of_month).guardado_cents
  ...
  test "the hero Save CTA appears only with a surplus and a savings account" do
    @user.transactions.create!(amount_cents: 500_00, occurred_on: Date.current, status: "posted",
