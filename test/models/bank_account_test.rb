...
class BankAccountTest < ActiveSupport::TestCase
  ...
  test "requires an institution" do
    account = BankAccount.new(account: @user.account)
    assert_not account.valid?
    assert account.errors.key?(:institution)
  end

  test "balance_reais parses into cents and formats back" do
    account = BankAccount.new(account: @user.account, institution: @inst, balance_reais: "1.500,00")
  ...
  test "balance stays unset when blank" do
    account = BankAccount.new(account: @user.account, institution: @inst, balance_reais: "")
  ...
  test "display_name uses the nickname, falling back to the institution" do
    account = BankAccount.new(account: @user.account, institution: @inst)
  ...
  test "derived balance adds signed posted rows created after the anchor" do
    account = BankAccount.create!(account: @user.account, institution: @inst, balance_cents: 100_000)
    other   = BankAccount.create!(account: @user.account, institution: @inst, balance_cents: 0)
    post = ->(attrs) { @user.account.transactions.create!({ status: "posted", occurred_on: Date.current }.merge(attrs)) }
  ...
  test "re-anchoring the balance absorbs rows recorded before the edit" do
    account = BankAccount.create!(account: @user.account, institution: @inst, balance_cents: 100_000)
    @user.account.transactions.create!(status: "posted", occurred_on: Date.current, direction: "expense",
