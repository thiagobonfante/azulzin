...
class CreditCardTest < ActiveSupport::TestCase
  ...
  def configured_card(limit: "1000")
    CreditCard.create!(user: @user, institution: @inst, credit_limit_reais: limit,
  ...
  test "a bare installment commitment (import) reserves its remaining parcels against the limit" do
    ...
    Commitment.create!(user: @user, credit_card: card, name: "Notebook 5x", kind: "installment",
  ...
  test "a subscription reserves the open bill's charge until the real charge posts" do
    card = configured_card
    sub = Commitment.create!(user: @user, credit_card: card, name: "Netflix", kind: "subscription",
    ...
    @user.transactions.create!(credit_card: card, commitment: sub, direction: "expense", status: "posted",
  ...
  test "a fanned-out card installment holds exactly the plan total (posted parcels, no double count)" do
    ...
    Installments::Create.call(user: @user, card: card, total_cents: 100_000, count: 10,
  ...
  test "open_bill_cents falls back to the manual snapshot on an unconfigured card" do
    card = CreditCard.new(user: @user, institution: @inst, current_bill_reais: "250")
