...
class MonthSummaryTest < ActiveSupport::TestCase
  ...
  def summary(month = @month) = MonthSummary.new(@user.account, month)

  def post!(**attrs)
    @user.account.transactions.create!({ amount_cents: 1_000, occurred_on: Date.new(2026, 7, 15),
  ...
  test "a checking→checking transfer changes only the two balances" do
    ...
    after = MonthSummary.new(@user.account, @month)
  ...
  test "an UNLINKED deposit within ±10% on the income's account also suppresses the expected term" do
    month = Date.current.beginning_of_month
    Income.create!(account: @user.account, bank_account: @account, name: "salário", amount_cents: 500_000,
                   schedule_kind: "fixed_day", schedule_day: 5)
  ...
  test "guardado counts transfers into a savings account, keyed by billing_month" do
    checking = BankAccount.create!(account: @user.account, institution: @inst, balance_cents: 100_000)
