...
class Imports::ApplyTest < ActiveSupport::TestCase
  ...
  test "topological apply: instruments first, then a commitment + income resolve the new account" do
    ...
    travel_to(Time.zone.local(2026, 7, 5)) { Imports::Apply.call(user: @user, accepted: accepted) }
    @user.reload

    assert_equal 1, @user.bank_accounts.count
    assert_equal 1, @user.incomes.count
    assert_equal 1, @user.commitments.count
    commitment = @user.commitments.first
    assert_equal @user.bank_accounts.first, commitment.bank_account
    assert_equal "import", commitment.source
    assert_nil commitment.source_message_id
    assert_equal @user.bank_accounts.first, @user.incomes.first.bank_account
    assert_equal "applied", import.reload.status
  end
  ...
  test "a dependent whose instrument was left unchecked fails with missing_instrument" do
    ...
    result = Imports::Apply.call(user: @user, accepted: { import.id => [ income_pid ] })
